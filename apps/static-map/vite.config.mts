...
// apps/static-map/dist/apps/static-map/... alongside the correct top-level dist/apps/static-map/. Absolute
// paths are invocation-cwd-independent, so both build passes land in the same place.
const APP_DIR = dirname(fileURLToPath(import.meta.url)); // <repo>/apps/static-map
const REPO_ROOT = resolve(APP_DIR, '../..'); // <repo>

export default defineConfig(() => {
  return {
    cacheDir: resolve(REPO_ROOT, 'node_modules/.vite/apps/static-map'),
    root: APP_DIR,
    build: {
      minify: false,
    },
    plugins: [
      ...
      qwikVite({
        debug: true,
        client: {
          outDir: resolve(REPO_ROOT, 'dist/apps/static-map/client'),
        },
        ssr: {
          outDir: resolve(REPO_ROOT, 'dist/apps/static-map/server'),
        },
