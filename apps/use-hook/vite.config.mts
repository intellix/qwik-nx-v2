...
// apps/use-hook/dist/apps/use-hook/... alongside the correct top-level dist/apps/use-hook/. Absolute
// paths are invocation-cwd-independent, so both build passes land in the same place.
const APP_DIR = dirname(fileURLToPath(import.meta.url)); // <repo>/apps/use-hook
const REPO_ROOT = resolve(APP_DIR, '../..'); // <repo>

export default defineConfig(() => {
  return {
    cacheDir: resolve(REPO_ROOT, 'node_modules/.vite/apps/use-hook'),
    root: APP_DIR,
    build: {
      minify: false,
    },
    plugins: [
      ...
      qwikVite({
        debug: true,
        client: {
          outDir: resolve(REPO_ROOT, 'dist/apps/use-hook/client'),
        },
        ssr: {
          outDir: resolve(REPO_ROOT, 'dist/apps/use-hook/server'),
        },
