...
// apps/qrl-static/dist/apps/qrl-static/... alongside the correct top-level dist/apps/qrl-static/. Absolute
// paths are invocation-cwd-independent, so both build passes land in the same place.
const APP_DIR = dirname(fileURLToPath(import.meta.url)); // <repo>/apps/qrl-static
const REPO_ROOT = resolve(APP_DIR, '../..'); // <repo>

export default defineConfig(() => {
  return {
    cacheDir: resolve(REPO_ROOT, 'node_modules/.vite/apps/qrl-static'),
    root: APP_DIR,
    build: {
      minify: false,
    },
    plugins: [
      ...
      qwikVite({
        debug: true,
        client: {
          outDir: resolve(REPO_ROOT, 'dist/apps/qrl-static/client'),
        },
        ssr: {
          outDir: resolve(REPO_ROOT, 'dist/apps/qrl-static/server'),
        },
