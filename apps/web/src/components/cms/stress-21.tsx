...
export const Stress21 = component$<Stress21Props>((props) => {
  const local = useSignal(21);
  const store = useStore({ n: 0, s: 'init' });

  useTask$(({ track }) => {
    track(() => props.count);
    console.log('[Stress21] task-1', props.count);
  });
  ...
  useVisibleTask$(() => {
    console.log('[Stress21] visible-1 mounted');
    return () => console.log('[Stress21] visible-1 cleanup');
  });

  // eslint-disable-next-line qwik/no-use-visible-task
  useVisibleTask$(({ track }) => {
    track(() => store.n);
    console.log('[Stress21] visible-2', store.n);
  });
